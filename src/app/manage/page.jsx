  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `https://edu-server-api.vercel.app/courses/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-gray-900 text-white">
      <div className="flex flex-col sm:flex-row justify-between items-start mb-6 sm:mb-8">
        ...
        <Link
          href="/add"
          className="flex items-center justify-center sm:justify-start space-x-2 px-4 py-2 bg-teal-500 font-semibold rounded-md shadow-lg hover:bg-teal-600 transition duration-150"
        >
              ...
              >
                ...
                <td className="py-3 text-right flex justify-end space-x-2">
                  <Link
                    ...
                    className="p-2 hover:text-teal-500 transition duration-150"
                  >
                    <FaEye />
                  </Link>
                  <button
                    onClick={() => handleDelete(course._id)}
                    className="p-2 bg-red-500 rounded-md hover:bg-red-600 transition duration-150"
                  >
                    <FaRegTrashAlt />
                  </button>
                </td>
